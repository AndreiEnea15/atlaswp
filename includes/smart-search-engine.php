...
function atlaswp_smart_search_posts($query_string){
    if(empty($query_string)) return new WP_Query(array('post_type'=>'post','posts_per_page'=>0));
...
$synonyms = array(
    ...
    // Get posts matching partial search
    ...
    $post_ids = array_map(function($item){ return $item['post']->ID; },$scored_posts);

    return new WP_Query(array('post_type'=>'post','post__in'=>$post_ids,'orderby'=>'post__in','posts_per_page'=>-1));
}
